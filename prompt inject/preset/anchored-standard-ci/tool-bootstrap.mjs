...
const ALLOWED_KEYS = new Set(['bootstrapTools', 'promoteOn', 'bootstrapMaxTokens', 'suppressedContextSources', 'compactionTools', 'residentTools'])
...
export function apply(ctx, config) {
  ...
  // Extra tools always present in the PROMOTED resident catalog, in addition
  // to the bootstrap pair and the discovery tools. Lets a deployment pin
  // host-registered tools (e.g. the vision plugin's see_image /
  // vision_set_key / vision_status) so the model does not have to unlock them
  // via dev_tool_search after promotion. Missing names are warned about and
  // skipped — never a full-catalog degrade.
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    try {
      ...
      if (status.promoted) {
        ...
        const keep = new Set([...bootstrapTools, ...RESIDENT_DISCOVERY_TOOLS, ...residentTools, ...unlockedFor(context.agent?.session)])
